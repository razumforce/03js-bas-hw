...
var answersHistory = []; // ДОБАВЛЕНО КАК ДЗ

generateNumber();
guessNumber();
checkAnswersHistory(); // ДОБАВЛЕНО КАК ДЗ
...
// ФУНКЦИИ НИЖЕ ДОБАВЛЕНЫ КАК ДЗ
function checkAnswersHistory() {
	if (answersHistory.length === 0) {
		alert("Ответов не было!");
	} else {
		alert('Всего было сделано ходов: ' + answersHistory.length);
		while (true) {
			if (wishToKnowHistory()) {
				var turn = getUserInputRange(1, answersHistory.length);
				console.log('Ваш ответ за ход ' + turn + ' был ' + answersHistory[turn-1]);
			} else {
				break;
			}
		}
...
function wishToKnowHistory() {
	while (true) {
		userInput = prompt('Хотите узнать ответ за какой-то из ходов? "y" - ДА, "q" - закончить игру');
		if (userInput === 'y') {
			return true;
		} else if (userInput === 'q') {
			return false;
		} else {
			alert('Введите либо "y", либо "q"');
		}
	}
}
